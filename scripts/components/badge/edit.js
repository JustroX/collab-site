app.controller("badgeEditController",function($scope,$http,$location,$timeout,session)
{
	let model = 
	{
		name: "", 
	...
	let api = 
	{
		...
		submit: function()
		{
			...
			api.error = '';
			api.loading = true;
			$http.put('/api/badge/'+api.view.target,$scope.model).then((res)=>
			{
				res = res.data;
				api.loaded(res);
				api.loading = false;
	...
	let view =
	{
		...
		load: function()
		{
			...
			{
				view.loading = true;
				$http.get('/api/badge/'+view.target+'?'+view.param).then((res)=>
				{
					view.load_options();
					res = res.data;
					view.loading = false;
